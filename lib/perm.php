...
class PERM
{
  ...
  ***************************************************************/  
  public static function setDefines($file)
  {
    ...
    $sessName = array_pop(explode("/",$file));
  ...
  ***************************************************************/  
  public function bit_or($bit1,$bit2)
  {
  ...
  /***************************************************************
    ...
    INPUTS:		bit -> bit string to operate on
              pos -> position to set
  ***************************************************************/  
  public function bit_set($bit,$pos)
  {
  ...
  ***************************************************************/  
  public function pad($bit,$len=null)
  {
  ...
  ***************************************************************/  
  private function userBitset()
  {
		...
		$sql = "SELECT bitmask FROM auth_accountperm WHERE account_id='".$this->accountId."';";
    ...
    $sql = "SELECT auth_grouplink.groupid AS groupid, auth_groupperm.bitmask AS bitmask FROM auth_grouplink
              LEFT JOIN auth_groupperm ON (auth_grouplink.groupid = auth_groupperm.group_id)	
              WHERE auth_grouplink.accountid='".$this->accountId."'";
    ...
    if (is_array($groupInfo["groupid"])) $group_string = "0,".implode(",",array_values(array_unique($groupInfo["groupid"])));
    else $group_string = "0";
  ...
  ***************************************************************/  
  ...
  {	
    ...
    $info = returnAccountInfo($this->accountId);
    ...
    $sql = "UPDATE auth_accountperm SET bitmask='$cb' WHERE account_id='".$this->accountId."'";
    $this->DB->query($sql);
  ...
  ***************************************************************/  
  public function saveAccount($level)
  {
    ...
    $sql = "SELECT bitmask,account_id FROM auth_accountperm WHERE account_id='".$this->accountId."'";
  ...
  ***************************************************************/  
  public function saveGroup($level)
  {
    ...
    $sql = "SELECT bitmask,group_id FROM auth_groupperm WHERE group_id='".$this->groupId."'";
    $info = $this->DB->single($sql);
    ...
    if ($info)
    {
      ...
      $this->DB->update("auth_groupperm",$opt);
    } 
    else 
    {
      $opt["group_id"] = $this->groupId;
      $this->DB->insert("auth_groupperm",$opt);
    }
