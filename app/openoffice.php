...
class OPENOFFICE {
  ...
  function __construct($path,$objId=null) 
  {
    ...
    else $this->ooName = array_pop(explode("/",$this->ooFile));
  ...
  function getExtension($filename) {
    ...
    $convname = array_pop(explode("/",$filename));
  ...
  ********************************************************************/
  //use username/password instead of session ids so openoffice
  //can access the images to do the conversion
  function fixHtmlImages($file) 
  {

    $loginstr = "login=".USER_LOGIN."&password=".USER_PASSWORD;

    $data = file_get_contents($file);
    $data = str_replace("[DOCMGR_SESSION_MARKER]",$loginstr,$data);
