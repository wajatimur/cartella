...
function PICKER(data,editor)
{
  ...
  var curelement = cursel.getSelectedElement();
	...
	this.run = function()
	{
		...
		if (EDITOR.config.image_mode=="docmgr") API = new this.DOCMGR();
		else API = new this.FILE();
	...
	this.addFile = function()
	{
		...
		var img = createImg(THEME_PATH + "/images/icons/close.png");
	...
	**************************************************************/
	this.FILE = function()
	{
		...
		this.upload = function()
		{
			...
			var url = SITE_URL + "app/fileupload.php?sessionId=" + SESSION_ID;
			url += "&filePath=" + EDITOR.config.image_path;

			form.action = url;
			form.target = "uploadframe";
			form.submit();

			updateSiteStatus("Uploading files");

			FILELIST.innerHTML = "<div class=\"successMessage\">Please Wait...</div>\n";
		...
		this.listFiles = function()
		{
			...
			FILELIST.innerHTML = "<div class=\"successMessage\">Please Wait...</div>";
			var url = SITE_URL + "/app/filelist.php?sessionId=" + SESSION_ID + "&filePath=" + EDITOR.config.image_path;
			protoReq(url,createMethodReference(this,"writeFileList"));
		...
		this.writeFileList = function(data)
		{
			...
			else if (!data.file) FILELIST.innerHTML = "<div class=\"errorMessage\">No Files Found</div>";
			else
			{
				...
				for (var i=0;i<data.file.length;i++)
				{
					...
					if (m=="list") FILELIST.appendChild(this.displayList(data.file[i]));
					else FILELIST.appendChild(this.displayThumb(data.file[i]));
		...
		this.displayList = function(obj)
		{
			...
			var fnlink = ce("a","","",obj.filename);
			fnlink.setAttribute("url",obj.fileurl);
			fnlink.onclick = createMethodReference(PIK,"setUrl");
			fndiv.appendChild(fnlink);

			//the options
			var rlimg = createImg(THEME_PATH + "/images/rotate_left.gif");
			rlimg.setAttribute("filepath",obj.filepath);
			rlimg.setAttribute("dir","left");
			rlimg.onclick = createMethodReference(this,"rotate");

			var rrimg = createImg(THEME_PATH + "/images/rotate_right.gif");
			rrimg.setAttribute("filepath",obj.filepath);
			rrimg.setAttribute("dir","right");
			rrimg.onclick = createMethodReference(this,"rotate");

			var delimg = createImg(THEME_PATH + "/images/trash.gif");
			delimg.setAttribute("filepath",obj.filepath);
		...
		this.displayThumb = function(obj)
		{
			...
			thumbimg.setAttribute("src",obj.thumburl);
			...
			thumbimg.setAttribute("url",obj.fileurl);
			thumbimg.onclick = createMethodReference(PIK,"setUrl");

			//the name
			var fndiv = ce("div","filename","",obj.filename);
		...
		this.rotate = function(e)
		{
			...
			var ref = getEventSrc(e);
			var fp = ref.getAttribute("filepath");
			var dir = ref.getAttribute("dir");

			FILELIST.innerHTML = "<div class=\"successMessage\">Please Wait...</div>";
			var url = SITE_URL + "/app/fileedit.php?sessionId=" + SESSION_ID + "&filePath=" + fp;
			url += "&editfile=rotate&dir=" + dir;
			protoReq(url,createMethodReference(this,"writeFileEdit"));
	...
	**************************************************************/
	this.DOCMGR = function()
	{
		...
		this.listFiles = function()
		{
			...
			p.add("mkdir","1");
			p.post(DOCMGR_API,createMethodReference(this,"writeFileList"));
		...
		this.writeFileList = function(data)
		{
			...
			if (data.error) alert(data.error);
			else if (!data.object) FILELIST.innerHTML = "<div class=\"errorMessage\">No Files Found</div>";
			else
			{
				...
				for (var i=0;i<data.object.length;i++)
				{
					...
					if (m=="list") FILELIST.appendChild(this.displayList(data.object[i]));
					else FILELIST.appendChild(this.displayThumb(data.object[i]));
		...
		this.rotate = function(e)
		{
			...
			p.add("direction",dir);
			p.add("object_id",id);
			p.post(DOCMGR_API,createMethodReference(this,"writeFileEdit"));
