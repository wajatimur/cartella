...
require_once("apilib/lib/docmgr/lib/setup.php");
...
class DOCMGR
{
	  	...
	  	if (!$this->permCheck("view"))
	  	{
			...
			else
			{
				...
				foreach ($idArr AS $idstr)
				{
					...
					$sql = "SELECT DISTINCT object_id FROM docmgr.dm_object_perm WHERE object_id IN (".$idstr.") AND ".permString();
			...
			//someone tried to sneaktard their way into an object
			if (!$pcheck)
			{
  ...
  /**********************************************************************
	...
	*********************************************************/
	...
	/*********************************************************
		...
		PURPOSE:	returns the id of an object specified by
							it's docmgr path
	*********************************************************/
	protected function objectFromPath($path) 
	{
	...
	*********************************************************/
	public function objectPathToId($path)
	{
		...
		$sql = "SELECT docmgr.path_to_id('".addslashes($path)."') AS objpath;";
		$info = $this->DB->single($sql);
	...
	*********************************************************/
	public function objectIdAllPaths($id=null,$list=null) 
	{
		...
		else 
		{
			...
			//get all paths to this object
			$ret =  $this->getObjIdPaths($list["object_id"][0],$list);
  ...
  *********************************************************************/
	function showCommon()
	{
		...
		if (!$this->objectId || is_array($this->objectId)) return false;
