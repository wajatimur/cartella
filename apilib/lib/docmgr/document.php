...
class DOCMGR_DOCUMENT extends DOCMGR_AOBJECT 
{
	...
	********************************************************************/
	public function get() 
	{
		...
		else if ($this->apidata["stream"])
		{
			...
			$this->PROTO->add("stream",fopen($filename,"r"));
	...
	***************************************************************************/
	protected function saveContent($data)
	{
		...
		if (!is_dir($documentPath)) recurMkDir($documentPath);
		...
		if (!$documentId) 
		{
			$this->throwError("Could not create document record");
			return false;
		}
		...
		{
			$this->throwError("Could not open document file");
			return false;		
		}
		...
		if ($this->DB->query($sql))
		 {
			...
			$pagesId = $this->getPagesFolder($this->objectId);

			if ($pagesId)
			{
				$o = new DOCMGR_OBJECT($pagesId);
				$o->delete();
			}
	...
	****************************************************************************/
	...
	public function getPagesFolder($objId=null,$nocreate=null) 
	{

		if (!$objId) $objId = $this->objectId;

		//the dot on the first character signifies it's hidden
		$name = ".object".$objId."_pages";

		//make sure this doesn't already exist.  if it does, return the object_id
		$sql = "SELECT object_id FROM docmgr.dm_view_objects WHERE name='".$name."' AND hidden='t'";
		$info = $this->DB->single($sql);
		
		if ($info || $nocreate) 
		{
			$returnId = $info["object_id"];
		} 
		else 
		{
