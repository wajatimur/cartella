...
class DOCMGR_UTIL_LOCK
{
	...
	************************************************************/
	public function get($objId=null)
	{
		...
		if ($objId) $obj = $objId;
		else $objId = $this->objectId;
		
		//clear any old locks
		$sql = "DELETE FROM docmgr.dm_locks WHERE 
							(created + timeout <= '".time()."') AND object_id IN (".$this->objectId.")";
		$this->DB->query($sql);

		//get locks (this should be any left over)
		$sql = "SELECT * FROM docmgr.dm_locks WHERE object_id IN (".$objId.")";
	...
	************************************************************/
	public function set($data=null)
	{
		...
		{
			$this->throwError("This object is locked by another user");
			return false;
		}
		...
		{
			$this->throwError("Cannot set an exclusive lock on a shared locked file");
			return false;
		}
		...
		else
		{
			...
			sendEventNotify($this->objectId,"OBJ_LOCK_ALERT");
	...
	************************************************************/
	function clear($token=null)
	{
		...
		else
		{
			...
			sendEventNotify($this->objectId,"OBJ_UNLOCK_ALERT");
	...
	public function validate($token=null)
	{
		...
		{
			//throw an error
			$this->throwError("Object is locked",OBJ_LOCKED);		
